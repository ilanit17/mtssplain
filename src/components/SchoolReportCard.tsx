...
import { HIERARCHICAL_CATEGORIES, CHALLENGES } from '../constants';
...
const DetailedSchoolReport: React.FC<DetailedSchoolReportProps> = ({ report }) => {
    ...
    const getTierInfo = (tier: number) => {
        switch (tier) {
            case 1: return { text: 'שכבה 1 - תפקוד תקין', color: '#27ae60' };
            case 2: return { text: 'שכבה 2 - תפקוד בינוני', color: '#f39c12' };
            case 3: return { text: 'שכבה 3 - התערבות אינטנסיבית', color: '#e74c3c' };
            default: return { text: 'לא נקבע', color: '#7f8c8d' };
        }
    };
    
    const tierInfo = getTierInfo(report.mtssTier);

    return (
        <div ref={reportRef} className="bg-white p-6 rounded-lg shadow-md font-sans">
            <div className="relative">
                <div className="absolute top-0 left-0 no-print">
                    <ExportControls targetRef={reportRef} reportName={`דוח-${school.name}`} />
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8 text-sm">
                ...
                <div className="bg-gray-100 p-3 rounded-md"><strong>סיווג MTSS:</strong> 
                    ...
                    <div className="bg-green-50 border-l-4 border-green-500 p-4 rounded-r-lg" role="alert">
                        <ul className="list-disc list-inside space-y-1 text-green-800">
                            {report.strengths.map((strength, index) => (
                                <li key={index} className="font-semibold">{strength}</li>
                            ))}
                        </ul>
                    </div>
                </div>
                    ...
                    <div key={category.name}>
                        ...
                        <div className="space-y-4">
                            {category.subCategories.map(subCat => {
                                ...
                                const challenges = (subCatAvg <= 3 && CHALLENGES[subCat.key]) ? CHALLENGES[subCat.key] : [];
